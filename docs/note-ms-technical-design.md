...
- 结论 2：当前后端运行形态以 JVM 为主，优先保证开发效率与生态稳定性。
...
- HTTP 服务：使用 Scala 3 on JVM 运行后端服务并监听端口。
- 业务逻辑：短链分配、内容保存、参数校验等纯 Scala 逻辑保持不变。
- 静态资源：在构建阶段把 Scala.js 产物打包进后端资源，由后端进程对外提供页面与静态文件。

### 2.2 取舍说明

- 生态稳定：JVM 生态更成熟，数据库与运维工具链更完整。
- 开发效率：编译/调试效率更高，适合 MVP 快速迭代。
- 存储选型：引入 PostgreSQL 后，系统本身就不是零依赖形态，JVM 方案更务实。

### 2.3 结论

- 推荐采用“JVM 后端 + Scala.js 前端 + PostgreSQL（乐观锁）”的多实例路线。
- Scala Native 可作为后续独立探索项，不纳入当前交付范围。
...
- `backend`（Scala 3 / JVM）：HTTP 路由、ID 分配、读写逻辑、静态资源服务。
- `shared`（跨端共享）：数据模型、请求/响应结构、校验规则。

### 3.2 运行形态

- 产物：基于 JVM 的后端应用包（可通过 `sbt` 运行或打包）。
- 前端交付：Scala.js 编译为 JS/CSS 后随后端资源一起分发。
- 启动方式：通过 JVM 启动后端进程对外提供完整服务。
...
- 输入边界：请求体上限 `256 KB`，内容上限 `200 KB`，超限返回 `413`。
- 领域建模：服务层统一通过 `NoteContent`（值对象）校验内容合法性，不直接透传裸 `String`。
- 存储写入：禁止字符串拼接 SQL，必须参数化查询。
- 输出编码：渲染到 HTML 时必须做 HTML 转义；渲染到 JSON 时使用 `circe` 编码。
- 日志策略：默认不记录完整正文，只记录长度、摘要和 `requestId`。
...
- M1：完成 Scala.js 页面 + JVM 后端最小读写链路。
- M2：完善错误处理、限流与日志指标。
- M3：优化构建流程（前端产物自动嵌入后端资源）与发布脚本。
